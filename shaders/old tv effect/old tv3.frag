...
{
 	return fract(sin(dot(uv, vec2(15.5151, 42.2561))) * 123421.14122 * sin(iTime * 0.13));   
}
...
void mainImage()
{
    ...
    vec2 crt_uv = crt_coords(uv, 7.);
    
    float s1 = scanline(uv, 1800., -10.);
    ...
    col = mix(col, vec4(s1 + s2), 0.03);
	fragColor = mix(col, vec4(noise(uv * 75.)), 0.1) * vignette(uv, .9, .6, 8.);;
