...
public var pixelNotesForBFm = false;
public var pixelNotesForDadm = true;
public var enablePixelUIm = false;
public var enablePixelGameOverm = false;
public var enableCameraHacksm = Options.week6PixelPerfect;
public var enablePauseMenum = true;
public var isSpookym = false;
...
function onNoteCreation(event) {
	if (event.note.strumLine == playerStrums && !pixelNotesForBFm) return;
	if (event.note.strumLine == cpuStrums && !pixelNotesForDadm) return;
	...
	if (event.note.isSustainNote) {
		...
		note.loadGraphic(Paths.image('stages/school/ui/arrows-pixelsmva'), true, 17, 17);
...
function onStrumCreation(event) {
	if (event.player == 1 && !pixelNotesForBFm) return;
	if (event.player == 0 && !pixelNotesForDadm) return;

	event.cancel();

	var strum = event.strum;
	strum.loadGraphic(Paths.image('stages/school/ui/arrows-pixelsmva'), true, 17, 17);
...
function postCreate() {
	if (enablePauseMenum) {
		PauseSubState.script = 'data/scripts/quickdraw-pause';
	}
